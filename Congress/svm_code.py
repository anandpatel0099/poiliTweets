...
vocab = {term: freq for term, freq in vocab.items() if freq > 6}
#vocab = dict()
...
Cs = range(1, 20)
clf = GridSearchCV(estimator=svc, param_grid=dict(C=Cs), cv = 7)
clf.fit(X, y)
...
tweets = []
for line in open('CNG_test.txt').readlines():
    tweets.append(line)
...
with open('labelled_CNG.txt','w') as f:
    idx=0
    for tweet in tweets:
